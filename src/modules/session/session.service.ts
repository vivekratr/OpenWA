...
export class SessionService implements OnModuleDestroy, OnModuleInit {
  ...
  // Pairing codes per session (mobile auth)
  ...
  async onModuleInit(): Promise<void> {
    ...
    const previouslyConnected = await this.sessionRepository.find({
      where: { connectedAt: Not(IsNull()) },
    });

    for (const session of previouslyConnected) {
      void this.start(session.id).catch(error => {
        this.logger.warn(`Failed to restore session: ${session.name}`, {
          sessionId: session.id,
          action: 'startup_restore_failed',
  ...
  async findByNameOrNull(name: string): Promise<Session | null> {
    return this.sessionRepository.findOne({ where: { name } });
  }

  async findOrCreateByPhone(phoneNumber: string): Promise<Session> {
    const normalized = this.normalizePhone(phoneNumber);
    const existing = await this.findByNameOrNull(normalized);
    if (existing) {
      return existing;
    }
    ...
    return this.create({
      name: normalized,
      config: { pairWithPhoneNumber: normalized },
    });
  }

  async startWithPhone(id: string, phoneNumber: string): Promise<Session> {
  ...
  private async initializeEngine(
    ...
    await engine.initialize({
      ...
      onPairingCode: (code: string): void => {
        ...
        void this.updateStatus(id, SessionStatus.PAIRING_READY);
      },
      ...
      onStateChanged: (engineState: EngineStatus): void => {
        const statusMap: Record<EngineStatus, SessionStatus> = {
          ...
          [EngineStatus.PAIRING_READY]: SessionStatus.PAIRING_READY,
  ...
  async getPairingCode(id: string): Promise<{ pairingCode: string; status: SessionStatus }> {
    ...
    if (!engine) {
      throw new BadRequestException('Session is not started. Call POST /mobile/auth/start first.');
    }

    const pairingCode = engine.getPairingCode() || this.pairingCodes.get(id);
