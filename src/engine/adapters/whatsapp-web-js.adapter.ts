...
export class WhatsAppWebJsAdapter extends EventEmitter implements IWhatsAppEngine {
    ...
    try {
      ...
      this.client = new Client({
        ...
        ...(this.config.pairWithPhoneNumber
          ? { pairWithPhoneNumber: { phoneNumber: this.config.pairWithPhoneNumber } }
          : {}),
        puppeteer: {
  ...
  private setupEventHandlers(): void {
    ...
    this.client.on('code', (code: string) => {
      ...
      this.setStatus(EngineStatus.PAIRING_READY);
  ...
  // ponytail: wwebjs sendMessage can return undefined even when WhatsApp delivers
  private toMessageResult(msg?: Message): MessageResult {
    if (!msg) {
      return { id: `sent_${Date.now()}`, timestamp: Math.floor(Date.now() / 1000) };
    }
    const id =
