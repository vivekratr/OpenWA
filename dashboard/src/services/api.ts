...
export interface ScheduledMessageDetail extends ScheduledMessageSummary {
...
export const scheduledMessageApi = {
  list: (sessionId: string, params?: { status?: ScheduledMessageStatus; limit?: number; offset?: number }) => {
    const query = new URLSearchParams();
    if (params?.status) query.set('status', params.status);
    if (params?.limit) query.set('limit', String(params.limit));
    if (params?.offset) query.set('offset', String(params.offset));
    const queryStr = query.toString();
    return request<{ data: ScheduledMessageSummary[]; total: number }>(
      `/sessions/${sessionId}/scheduled-messages${queryStr ? `?${queryStr}` : ''}`,
    );
  },
  get: (sessionId: string, id: string) =>
    request<ScheduledMessageDetail>(`/sessions/${sessionId}/scheduled-messages/${id}`),
  create: (
    sessionId: string,
    data: {
      scheduledAt: string;
    ...
    request<ScheduledMessageSummary>(`/sessions/${sessionId}/scheduled-messages`, {
      method: 'POST',
      body: JSON.stringify(data),
    }),
  cancel: (sessionId: string, id: string) =>
    request<{ id: string; status: ScheduledMessageStatus; completedAt?: string }>(
      `/sessions/${sessionId}/scheduled-messages/${id}`,
      { method: 'DELETE' },
    ),
};
